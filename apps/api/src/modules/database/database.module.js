            ...
            providers: [
                {
                    ...
                    useFactory: function (config) {
                        var pool = new pg_1.Pool({
                            connectionString: config.getOrThrow('app.DATABASE_URL'),
                            ssl: config.get('app.NODE_ENV') === 'production' ? { rejectUnauthorized: true } : false,
                        });
                        return (0, node_postgres_1.drizzle)(pool, { schema: schema });
                    },
                },
            ],
